...
if(FALSE) {
    ## If you want to see all the data plotted...
    par(ask = TRUE)
...
X.const <- returns(SP500.const) # compute -log-returns
...
mod <- lm(t(X) ~ X.sectors - 1) # -1: no intercept is required
B <- model.matrix(mod)

## Factors can be constructed from the coefficient matrix
coef.mat <- t(coef(mod))
...
sort(eps.variances) # display them from smallest to largest
...
cor(F)
eps <- t(residuals(mod))
## This matrix should be near-diagonal for a perfect factor model.
## Of course this won't be the case in practice
cor.eps <- cor(eps)
## Is Cor(eps) (roughly) diagonal? Check upper corner
matrix_plot(cor.eps[1:75, 1:75], ran = c(-1, 1))
...
err <- P - cor(X)
matrix_plot(err[1:75,1:75], ran = c(-2, 2))
