...
plot(maturities, yields[day,], main = as.character(thedate), ylim = range(yields[day], fitted),
     xlab = "Time to maturity", ylab = "Yield", pch = 20)
lines(maturities, fitted)


k2 <- function(s, eta) (1-exp(-eta*s)) / (eta*s)
k3 <- function(s,eta) k2(s,eta)-exp(-eta*s)
