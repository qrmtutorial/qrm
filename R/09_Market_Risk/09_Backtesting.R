...
spec.n <- ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(1,1)),mean.model=list(armaOrder=c(1,1),include.mean=TRUE),distribution.model="norm")
spec.t <- ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(1,1)),mean.model=list(armaOrder=c(1,1),include.mean=TRUE),distribution.model="std")
spec.tskew <- ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(1,1)),mean.model=list(armaOrder=c(1,1),include.mean=TRUE),distribution.model="sstd")


# We now carry out a series of rolling out-of-sample VaR predictions
# We refit the model every 10 days

roll.n <- ugarchroll(spec.n,gains.d,n.start=1000,window.size=1000,refit.every=10,refit.window="moving",solver="hybrid",calculate.VaR=TRUE,VaR.alpha=c(0.05,0.01))
show(roll.n)
plot(roll.n,which=4)
...
roll.t <- ugarchroll(spec.t,gains.d,n.start=1000,window.size=1000,refit.every=10,refit.window="moving",solver="hybrid",calculate.VaR=TRUE,VaR.alpha=c(0.05,0.01))
show(roll.t)
plot(roll.t,which=4)
...
roll.tskew <- ugarchroll(spec.tskew,gains.d,n.start=1000,window.size=1000,refit.every=10,refit.window="moving",solver="hybrid",calculate.VaR=TRUE,VaR.alpha=c(0.05,0.01))
show(roll.tskew)
